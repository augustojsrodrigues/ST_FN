# -*- coding: utf-8 -*-
"""
Streamlit app for a modified-opportunistic inspection policy with false negative errors.
...
Author: Augusto J. S. Rodrigues and collaborators
"""
...
# ---------------------------------------------------------------------
st.set_page_config(
    page_title="Opportunistic Inspection Policy",
    page_icon="🛠️",
    layout="wide",
...
def run_policy_simulation(inputs: PolicyInputs) -> Tuple[Dict[str, float], pd.DataFrame]:
    ...
    The logic follows the 20-case simulation structure:
    - X is the time to defect;
    - H is the delay time from defect to failure;
    - Z is the first accepted opportunity after threshold S;
    - beta_s is the false negative probability for scheduled inspections;
    - beta_o is the false negative probability for opportunistic inspections.
    """
    ...
    for _ in range(n):
        ...
        # Case 20
            ...
            # Numerical safety fallback. It should not be reached if the 20 cases are exhaustive.
            cost += CF
...
st.sidebar.caption("Insert the policy values and system parameters. The app evaluates the policy without optimization.")
...
# Main layout
# ---------------------------------------------------------------------
col_title, col_gif = st.columns([0.78, 0.22], vertical_alignment="center")
with col_title:
    st.markdown(
        """
        <div class="hero">
            ...
            <span class="badge">False negatives</span>
            <h1>Modified-opportunistic inspection policy</h1>
            <p>Evaluate a fixed pair <b>(S, T)</b> for a critical system under scheduled and opportunistic inspections with false negative errors.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
with col_gif:
    st.image("assets/maintenance_loop.gif", use_container_width=True)

st.markdown(
    """
    <div class="card">
    <b>Policy structure.</b> The user defines the threshold <b>S</b>, from which opportunities may be accepted, and the scheduled inspection interval <b>T</b>. The app then estimates the cost rate, MTBOF, PFRBO, LOM, and the probability of each event case.
    </div>
    """,
    unsafe_allow_html=True,
)
...
if c_f < c_p:
    st.warning("CF is smaller than CP. This is allowed for testing, but it is unusual because corrective replacement is normally more expensive than preventive replacement.")
...
st.markdown('<div class="success-box"><b>Simulation completed.</b> Results below evaluate the selected policy only. No optimization was performed.</div>', unsafe_allow_html=True)
...
with col_a:
    st.markdown(
        ...
        <div class="card">
        <b>Cost rate</b><br>
        Lower values indicate a cheaper policy per unit of operating time. Compare this value across different choices of S and T.
        </div>
        """,
        unsafe_allow_html=True,
    )
with col_b:
    st.markdown(
        ...
        <div class="card">
        <b>PFRBO</b><br>
        Higher values indicate that opportunities are effectively preventing potential failures through preventive replacement.
        </div>
        """,
        unsafe_allow_html=True,
    )
with col_c:
    st.markdown(
        ...
        <div class="card">
        <b>LOM</b><br>
        Higher values indicate that opportunities exist, but their value is being lost because defects are missed by inspection errors.
        </div>
        """,
        unsafe_allow_html=True,
    )
